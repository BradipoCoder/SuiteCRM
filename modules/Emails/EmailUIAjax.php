...
if (isset($_REQUEST['emailUIAction'])) {
    switch ($_REQUEST['emailUIAction']) {
        ...
        case "composeEmail":
            ...
            if (isset($_REQUEST['sugarEmail']) && $_REQUEST['sugarEmail'] == 'true' && isset($_REQUEST['uid'])
                && !empty($_REQUEST['uid'])
            ) {
            ...
            elseif (isset($_REQUEST['uid']) && !empty($_REQUEST['uid']) && isset($_REQUEST['ieId'])
                    && !empty($_REQUEST['ieId'])
            ) {
        ...
        case "getRelateForm":
            if (isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
                ...
                $ret = $email->et->getImportForm(
                    array('showTeam' => FALSE, 'showAssignTo' => FALSE, 'showDelete' => FALSE), $email, 'RelateEditView'
                );
                $out = trim($json->encode($ret, FALSE));
                echo $out;
            }
        ...
        case "relateEmails":
            if (isset($_REQUEST['uid']) && !empty($_REQUEST['uid']) && isset($_REQUEST['parent_id'])
        ...
        case "doAssignmentAssign":
            $out = $email->et->doAssignment(
                $_REQUEST['distribute_method'], $_REQUEST['ieId'], $_REQUEST['folder'], $_REQUEST['uids'],
                $_REQUEST['users']
            );
            echo $out;
            break;
        case "doAssignmentDelete";
            ...
            if (isset($_REQUEST['uids']) && !empty($_REQUEST['uids']) && isset($_REQUEST['ieId'])
                && !empty($_REQUEST['ieId'])
        ...
        case "markEmail":
            ...
            if (isset($_REQUEST['uids']) && !empty($_REQUEST['uids']) && isset($_REQUEST['type'])
                && !empty($_REQUEST['type'])
                && isset($_REQUEST['folder'])
                && !empty($_REQUEST['folder'])
                && isset($_REQUEST['ieId'])
                && (!empty($_REQUEST['ieId'])
                    || (empty($_REQUEST['ieId'])
                        && strpos($_REQUEST['folder'], 'sugar::') !== FALSE))
            ) {
                ...
                $ret = array();
                if (strpos($_REQUEST['folder'], 'sugar::') !== FALSE && ($_REQUEST['type'] == 'deleted')
                    && !ACLController::checkAccess('Emails', 'delete')
                ) {
                    $ret['status'] = FALSE;
        ...
        case "checkEmail":
            ...
            elseif (isset($_REQUEST['all']) && !empty($_REQUEST['all'])) {
                ...
                $GLOBALS['log']->info(
                    "[EMAIL] - checkEmail found " . count($showFolders)
                    . " accounts to check for user [{$current_user->user_name}]"
                );
        ...
        case "importEmail":
            ...
            if (strpos($_REQUEST['uid'], $app_strings['LBL_EMAIL_DELIMITER']) !== FALSE) {
                ...
                foreach ($exUids as $msgNo) {
                    ...
                    $return[] = $app_strings['LBL_EMAIL_MESSAGE_NO'] . " " . $count . ", " . $app_strings['LBL_STATUS']
                                . " "
                                . ($status ? $app_strings['LBL_EMAIL_IMPORT_SUCCESS'] : $app_strings['LBL_EMAIL_IMPORT_FAIL']);
                    $count++;
                    if (($_REQUEST['delete'] == 'true') && $status
        ...
        case "deleteMessage":
            ...
            if (isset($_REQUEST['uid']) && !empty($_REQUEST['uid']) && isset($_REQUEST['ieId'])
                && !empty($_REQUEST['ieId'])
            ) {
                $ie->retrieve($_REQUEST['ieId']);
                $ie->mailbox = $_REQUEST['mbox'];
        ...
        case "getSingleMessageFromSugar":
            ...
            if (isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
                ...
                if ($email->status == 'draft' || $email->type == 'draft') {
                    ...
                    $out['meta']['email']['toaddrs'] = $email->et->generateExpandableAddrs(
...
eoq;
            ...
            if (isset($_REQUEST['uid']) && !empty($_REQUEST['uid']) && isset($_REQUEST['ieId'])
                ...
                foreach ($exUids as $k => $uid) {
                    if ($email->et->validCacheFileExists(
                        $_REQUEST['ieId'], 'messages', $_REQUEST['mbox'] . $uid . ".php"
                    )
                    ) {
                        $msg = $email->et->getCacheValue(
                            $_REQUEST['ieId'], 'messages', $_REQUEST['mbox'] . $uid . ".php", 'out'
                        );
                    }
                    else {
                        $ie->retrieve($_REQUEST['ieId']);
                        ...
                        $email->et->writeCacheFile(
                            'out', $msg, $_REQUEST['ieId'], 'messages', "{$_REQUEST['mbox']}{$uid}.php"
                        );
                    }
        ...
        case "getMessageListSugarFolders":
            ...
            if (isset($_REQUEST['ieId']) && !empty($_REQUEST['ieId'])) {
                ...
                if (!empty($sortSerial) && !empty($_REQUEST['ieId']) && !empty($_REQUEST['mbox'])) {
                    ...
                    $GLOBALS['log']->debug(
                        "********** EMAIL 2.0********** ary=" . print_r($sortArray, TRUE) . ' id=' . $_REQUEST['ieId']
                        . '; box=' . $_REQUEST['mbox']
                    );
                ...
                if (!empty($_REQUEST['sort']) && !empty($_REQUEST['dir'])) {
                    $email->et->saveListViewSortOrder(
                        $_REQUEST['ieId'], $_REQUEST['mbox'], $_REQUEST['sort'], $_REQUEST['dir']
                    );
                    $sort = $_REQUEST['sort'];
        ...
        case "moveEmails":
            ...
            $ie->moveEmails(
                $_REQUEST['sourceIeId'], $_REQUEST['sourceFolder'], $_REQUEST['destinationIeId'],
        ...
        case "renameFolder":
            ...
            if (isset($_REQUEST['ieId']) && isset($_REQUEST['oldFolderName']) && !empty($_REQUEST['oldFolderName'])
                && isset($_REQUEST['newFolderName'])
                && !empty($_REQUEST['newFolderName'])
            ) {
                $ie->retrieve($_REQUEST['ieId']);
                $ie->renameFolder($_REQUEST['oldFolderName'], $_REQUEST['newFolderName']);
            }
            elseif (isset($_REQUEST['folderId']) && !empty($_REQUEST['folderId']) && isset($_REQUEST['newFolderName'])
                    && !empty($_REQUEST['newFolderName'])
            ) {
        ...
        case "moveFolder":
            ...
            if (isset($_REQUEST['folderId']) && !empty($_REQUEST['folderId']) && isset($_REQUEST['newParentId'])
                && !empty($_REQUEST['newParentId'])
                && $_REQUEST['newParentId'] != $_REQUEST['folderId']
            ) {
        ...
        case "saveListViewSortOrder":
            ...
            $email->et->saveListViewSortOrder(
                $_REQUEST['ieId'], $_REQUEST['focusFolder'], $_REQUEST['sortBy'], $_REQUEST['reverse']
            );
            break;
        ////    END FOLDER ACTIONS
        ///////////////////////////////////////////////////////////////////////////
        ...
        case "testOutbound":
            ...
            $out = $email->sendEmailTest(
                $_REQUEST['mail_smtpserver'], $_REQUEST['mail_smtpport'], $_REQUEST['mail_smtpssl'],
        ...
        case "search":
            ...
            if (isset($_REQUEST['subject']) && !empty($_REQUEST['subject']) && isset($_REQUEST['ieId'])
                && !empty($_REQUEST['ieId'])
            ) {
        ...
        case "addContact":
            ...
            if (isset($_REQUEST['bean_module']) && !empty($_REQUEST['bean_module']) && isset($_REQUEST['bean_id'])
                && !empty($_REQUEST['bean_id'])
            ) {
        ...
        case "getAddressSearchResults":
            ...
            if (!empty($qArray['query'])) {
                ...
                $direction = !empty($_REQUEST['dir'])
                             && in_array(
                                 strtolower($_REQUEST['dir']), array("asc", "desc")
                             ) ? $_REQUEST['dir'] : "asc";
