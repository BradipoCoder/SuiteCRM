...
class EmailMan extends SugarBean
{
    ...
    /** @var array  */
    public $restricted_addresses = array();

    /** @var array  */
    public $restricted_domains = array();

    /** @var  \InboundEmail */
    public $current_mailbox;

    /** @var  string */
    public $mailbox_from_addr;

    /** @var array  */
    public $tracker_urls = array();

    /** @var bool  */
    public $has_optout_links = FALSE;
    ...
    /**
     * @deprecated deprecated since version 7.6, PHP4 Style Constructors are
     *   deprecated and will be remove in 7.8, please update your code, use
     *   __construct instead
     */
     ...
     * @param string $parentbean
     * @param bool   $singleSelect
     * @param bool   $ifListForExport
     *
     * @return array|string
     */
    public function create_new_list_query($order_by, $where, $filter = array(), $params = array(), $show_deleted = 0,
                                   $join_type = '', $return_array = FALSE, $parentbean = NULL, $singleSelect = FALSE,
                                   $ifListForExport = FALSE)
    {
        ...
        $query['select'] = "SELECT $this->table_name.* ,
					...
					(CASE related_type
						WHEN 'Contacts' THEN " . $this->db->concat(
                'contacts', array(
                'first_name',
                'last_name'
            ), '&nbsp;'
            ) . "
						WHEN 'Leads' THEN " . $this->db->concat(
                'leads', array(
                'first_name',
                'last_name'
            ), '&nbsp;'
						...
						WHEN 'Users' THEN " . $this->db->concat(
                'users', array(
                'first_name',
                'last_name'
            ), '&nbsp;'
            ) . "
						WHEN 'Prospects' THEN " . $this->db->concat(
                'prospects', array(
                'first_name',
                'last_name'
            ), '&nbsp;'
     ...
     * @return String
     */
    public function create_queue_items_query($order_by, $where, $filter = array(), $params = array(), $show_deleted = 0,
                                      $join_type = '', $return_array = FALSE, $parentbean = NULL, $singleSelect = FALSE)
    {
    ...
    function get_list_view_data()
    {
        ...
        $query = "SELECT addr.email_address FROM email_addresses addr,email_addr_bean_rel eb WHERE eb.deleted=0 AND addr.id=eb.email_address_id AND bean_id ='"
                 . $related_id . "' AND primary_address = '1'";
    ...
    function set_as_sent($email_address, $delete = TRUE, $email_id = NULL, $email_type = NULL, $activity_type = NULL)
    {
        ...
        else
        {
            ...
            $query = 'UPDATE ' . $this->table_name
                     . " SET in_queue='1', send_attempts='$this->send_attempts', in_queue_date=" . $this->db->now()
                     . " WHERE id = $this->id";
            $this->db->query($query);
        }
    }
    ...
    /**
     * Function finds the reference email for the campaign. Since a campaign can have
     * multiple email templates the reference email has same id as the marketing id.
     * this function will create an email if one does not exist. also the function
     * will load these relationships leads, accounts, contacts users and targets
     *
     * @param string $marketing_id
     * @param string $subject
     * @param string $body_text
     * @param string $body_html
     * @param string $campagin_name
     * @param string $from_address
     * @param string $sender_id
     * @param array $notes
     * @param array $macro_nv
     * @param bool $newmessage
     * @param string $from_address_name
     *
     * @return string
     */
    function create_ref_email($marketing_id, $subject, $body_text, $body_html, $campagin_name, $from_address,
                              $sender_id, $notes, $macro_nv, $newmessage, $from_address_name)
    {
        ...
        if ($newmessage or empty($this->ref_email->id))
        {
            ...
            $upd_ref_email = (!empty($this->ref_email->id) and $this->ref_email->parent_type == 'test'
                                                               and $this->ref_email->parent_id == 'test');
            //following condition is for switching back to test mode.
            if (!$upd_ref_email)
            {
                $upd_ref_email = ($this->test and !empty($this->ref_email->id) and empty($this->ref_email->parent_type)
                                                                                   and empty($this->ref_email->parent_id));
            }
                ...
                foreach ($notes as $note)
                {
                    if ($note->object_name == 'Note')
                    {
                        if (!empty($note->file->temp_file_location)
                            && is_file($note->file->temp_file_location)
                        )
                        {
                    ...
                    if(isset($filename) && isset($mime_type)) {
                        $noteAudit = new Note();
     ...
     *
     * @param \Account|\Contact|\Lead|\Prospect|\User $module
    ...
    /**
     * Verifies the email_marketing message and email_template configured
     * for the campaign. If issues are found a fatal error will be logged but processing will not stop.
     * Returns true if all campaign parameters are set correctly
     *
     * @param string $marketing_id
     * @return bool
     */
    function verify_campaign($marketing_id)
    {
        if (!isset($this->verified_email_marketing_ids[$marketing_id]))
        {
            ...
            {
                $GLOBALS['log']->fatal(
                    'Error retrieving marketing message for the email campaign. marketing_id = ' . $marketing_id
                );
                return FALSE;
            }
            ...
            {
                $GLOBALS['log']->fatal(
                    'Error retrieving template for the email campaign. marketing_id = ' . $marketing_id
                );
                return FALSE;
            }
            ...
            {
                $GLOBALS['log']->fatal(
                    'Error retrieving template for the email campaign. template_id = ' . $email_marketing->template_id
                );
                return FALSE;
            }
            ...
            {
                $GLOBALS['log']->fatal(
                    'Email template is empty. email_template_id=' . $email_marketing->template_id
                );
                return FALSE;
            }
    ...
    function sendEmail($mail, $save_emails = 1, $testmode = FALSE)
    {
        ...
        if (!isset($this->tracking_url))
        {
            ...
            if (isset($admin->settings['massemailer_tracking_entities_location_type'])
                and $admin->settings['massemailer_tracking_entities_location_type'] == '2'
                    and isset($admin->settings['massemailer_tracking_entities_location'])
            )
            {
        ...
        /** @var \Account|\Contact|\Lead|\Prospect|\User $module */
        $module = new $class();
        $module->retrieve($this->related_id);

        /** @var \EmailAddress $moduleEmailAddress */
        $moduleEmailAddress = $module->emailAddress;
        $moduleEmailAddress->handleLegacyRetrieve($module);
        ...
        if (!$this->is_primary_email_address($module))
        {
            ...
            $GLOBALS['log']->fatal(
                'Email Address provided is not Primary Address for email with id ' . $module->email1
                . "' Emailman id=$this->id"
            );
            return TRUE;
        }
        ...
        if (!$this->valid_email_address($module->email1))
        {
            ...
            $GLOBALS['log']->fatal(
                'Encountered invalid email address' . $module->email1 . " Emailman id=$this->id"
            );
            return TRUE;
        }
        ...
        )
        {
            ...
            $dup_query = "select id from campaign_log where more_information='" . $this->db->quote($module->email1)
                         . "' and marketing_id='" . $this->marketing_id . "'";
            ...
            if (empty($this->current_emailtemplate) or $this->current_emailtemplate->id
                                                       !== $this->current_emailmarketing->template_id
            )
            {
            ...
            if (empty($this->current_mailbox->id) or $this->current_mailbox->id
                                                     !== $this->current_emailmarketing->inbound_email_id
            )
            {
            ...
            if (empty($this->current_campaign->id) or $this->current_campaign->id
                                                      !== $this->current_emailmarketing->campaign_id
            )
            {
            ...
            $tracker_url_template = $this->tracking_url . 'index.php?entryPoint=campaign_trackerv2&track=%s'
                                    . '&identifier=' . $this->target_tracker_key;
            $removeme_url_template = $this->tracking_url . 'index.php?entryPoint=removeme&identifier='
                                     . $this->target_tracker_key;
            ...
            $text_only = FALSE;
            if (isset($this->current_emailtemplate->text_only)
                && $this->current_emailtemplate->text_only
            )
            {
                $text_only = TRUE;
            }
            ...
            else
            {
                ...
                if ($btracker && isset($tracker_url) && isset($tracker_text))
                {
                ...
                else
                {
                    if (!empty($tracker_url))
                    {
                        $mail->Body = str_replace(
                            'TRACKER_URL_START', "<a href='" . $tracker_url . "'>", $mail->Body
                        );
                        $mail->Body = str_replace('TRACKER_URL_END', "</a>", $mail->Body);
                        $mail->AltBody = str_replace(
                            'TRACKER_URL_START', "<a href='" . $tracker_url . "'>", $mail->AltBody
                        );
                ...
                {
                    $mail->Body .= "<br /><span style='font-size:0.8em'>{$mod_strings['TXT_REMOVE_ME']} <a href='"
                                   . $this->tracking_url
                                   . "index.php?entryPoint=removeme&identifier={$this->target_tracker_key}'>{$mod_strings['TXT_REMOVE_ME_CLICK']}</a></span>";
                }
                ...
                if ($btracker && isset($tracker_url))
                {
                    $mail->AltBody .= "\n" . $tracker_url;
                }
                if ($this->has_optout_links == FALSE)
                {
                    $mail->AltBody .= "\n\n\n{$mod_strings['TXT_REMOVE_ME_ALT']} " . $this->tracking_url
                                      . "index.php?entryPoint=removeme&identifier=$this->target_tracker_key";
                }
            ...
            if ($success)
            {
                ...
                else
                {
                    ...
                    $email_id = $this->create_ref_email(
                        $this->marketing_id, $this->current_emailtemplate->subject, $this->current_emailtemplate->body, $this->current_emailtemplate->body_html, $this->current_campaign->name, $this->mailbox_from_addr, $this->user_id, $this->notes_array, $macro_nv, $this->newmessage, $fromAddressName
                    );
    ...
    function is_primary_email_address($bean)
    {
        ...
        $primary_qry = "select email_address_id from email_addr_bean_rel where bean_id = '" . $bean->id
