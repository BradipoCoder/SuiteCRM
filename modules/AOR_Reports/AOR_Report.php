...
class AOR_Report extends Basic
{
  ...
  function build_report_chart($chartIds = NULL, $chartType = self::CHART_TYPE_PCHART)
  {
    ...
    $sql = "SELECT id FROM aor_fields WHERE aor_report_id = '" . $this->id
    ...
    else if (!$rows)
    {
      if ($path)
      {
        ...
        foreach ($path as $pth)
        {
          ...
          $query = "SELECT $_fieldIdName FROM " . $this->db->quoteIdentifier($pth['module_path'][0])
  ...
  function build_group_report($offset = -1, $links = TRUE, $extra = array())
  {
    ...
    if ($field_id != '')
    {
      ...
      if ($data['type'] == 'currency' && !stripos($field->field, '_USD')
      ...
      )
      {
        ...
        {
          $query['select'][$table_alias . '_currency_id'] = $table_alias . '_cstm' . ".currency_id AS '" . $table_alias
                                                            . "_currency_id'";
        }
        else
        {
          $query_array['select'][$table_alias . '_currency_id'] = $table_alias . ".currency_id AS '" . $table_alias
                                                                  . "_currency_id'";
        }
      }
      ...
      if ((isset($data['source']) && $data['source'] == 'custom_fields'))
      {
        ...
        $query_array = $this->build_report_query_join(
          $table_alias . '_cstm', $table_alias . '_cstm', $table_alias, $field_module, 'custom', $query
        );
      }
      else
      {
  ...
  function build_report_html($offset = -1, $links = TRUE, $group_value = '', $tableIdentifier = '', $extra = array())
  {
    ...
    $html = "<table class='list' id='report_table" . $group_value
    ...
    if ($offset >= 0)
    {
      ...
      else
      {
        $html .= "<button type='button' id='listViewStartButton_top' name='listViewStartButton' title='Start' class='button' onclick='changeReportPage(\""
                 . $this->id . "\",0,\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
                ...
                <button type='button' id='listViewPrevButton_top' name='listViewPrevButton' class='button' title='Previous' onclick='changeReportPage(\""
                 . $this->id . "\"," . $previous_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
      ...
      if ($next_offset < $total_rows)
      {
        $html .= "<button type='button' id='listViewNextButton_top' name='listViewNextButton' title='Next' class='button' onclick='changeReportPage(\""
                 . $this->id . "\"," . $next_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
                 ...
                 . $this->id . "\"," . $last_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
  ...
  function build_report_query($group_value = '', $extra = array())
  {
    ...
    echo "<br />QUERY-ARRAY: " . print_r($query_array, TRUE);
    ...
    if ($beanList[$this->report_module])
    {
      ...
      while ($row = $this->db->fetchByAssoc($result))
      {
        ...
        if ($data['type'] == 'relate' && isset($data['id_name']))
        {
          ...
          if (isset($data_new['source']) && $data_new['source'] == 'non-db' && $data_new['type'] != 'link'
              && isset($data['link'])
          )
          {
            $data_new['type'] = 'link';
        ...
        if ($data['type'] == 'currency' && isset($field_module->field_defs['currency_id']))
        {
          ...
          echo "<br />ADDING FORCED CURRENCY_ID GROUP FIELD: " . $this->db->quoteIdentifier($table_alias . '_cstm') . "
          .currency_id";
          ...
          {
            $query['select'][$table_alias . '_currency_id'] = $this->db->quoteIdentifier($table_alias . '_cstm')
                                                              . ".currency_id AS '" . $table_alias . "_currency_id'";
            $query['second_group_by'][] = $this->db->quoteIdentifier($table_alias . '_cstm') . ".currency_id";
          }
          else
          {
            $query['select'][$table_alias . '_currency_id'] = $this->db->quoteIdentifier($table_alias)
                                                              . ".currency_id AS '" . $table_alias . "_currency_id'";
        ...
        if ($field->group_by == 1)
        {
          echo "<br />ADDING PRIMARY GROUP FIELD: " . $select_field;
          $query['group_by'][] = $field->format ? str_replace(
        ...
        {
          /*
          echo "<br />ADDING SECONDARY GROUP FIELD: " . $select_field;
          $query['second_group_by'][] = $field->format ? str_replace(
            '(%1)', '(' . $select_field . ')', preg_replace(
            array(
              '/\s+/',
              '/Y/',
  ...
  function build_report_query_join($name, $alias, $parentAlias, SugarBean $module, $type, $query = array(),
                                   SugarBean $rel_module = NULL)
  {
    ...
    if (!isset($query['join'][$alias]))
    {
      ...
      {
        case 'custom':
          $query['join'][$alias] = 'LEFT JOIN ' . $this->db->quoteIdentifier($module->get_custom_table_name()) . ' '
                                   . $this->db->quoteIdentifier($name) . ' ON '
                                   . $this->db->quoteIdentifier($parentAlias) . '.id = '
                                   . $this->db->quoteIdentifier($name) . '.id_c ';
          break;
  ...
  function build_report_query_where($query = array())
  {
    ...
    if ($beanList[$this->report_module])
    {
      ...
      $sql = "SELECT id FROM aor_conditions WHERE aor_report_id = '" . $this->id
      ...
      while ($row = $this->db->fetchByAssoc($result))
      {
        ...
        if (isset($app_list_strings['aor_sql_operator_list'][$condition->operator]))
        {
          ...
          if ($data['type'] == 'relate' && isset($data['id_name']))
          {
            ...
            if (!empty($data_new['source']) && $data_new['source'] == 'non-db' && $data_new['type'] != 'link'
                && isset($data['link'])
            )
            {
              $data_new['type'] = 'link';
          ...
          if ((isset($data['source']) && $data['source'] == 'custom_fields'))
          {
            ...
            $query = $this->build_report_query_join(
              $table_alias . '_cstm', $table_alias . '_cstm', $oldAlias, $condition_module, 'custom', $query
            );
          }
          else
          {
          ...
          switch ($condition->value_type)
          {
            case 'Field':
              ...
              if ((isset($data['source']) && $data['source'] == 'custom_fields'))
              {
                ...
                $query = $this->build_report_query_join(
                  $condition_module->table_name . '_cstm', $table_alias
                                                           . '_cstm', $table_alias, $condition_module, 'custom', $query
                );
              }
              else
              {
              ...
              if ($params[1] != 'now')
              {
                switch ($params[3])
                {
                  ...
                  default:
                    if ($sugar_config['dbconfig']['db_type'] == 'mssql')
                    {
                      $value = "DATEADD(" . $params[3] . ",  " . $app_list_strings['aor_date_operator'][$params[1]]
                               . " $params[2], $value)";
                    }
                    else
                    {
                      $value = "DATE_ADD($value, INTERVAL " . $app_list_strings['aor_date_operator'][$params[1]]
                               . " $params[2] " . $params[3] . ")";
                    }
                    break;
                }
            ...
            case 'Multi':
              ...
              if (!empty($multi_values))
              {
                ...
                foreach ($multi_values as $multi_value)
                {
                  ...
                  $value .= $field . ' ' . $app_list_strings['aor_sql_operator_list'][$condition->operator] . " '"
                            . $multi_value . "'";
                }
                $value .= ')';
              }
          ...
          if (!$where_set)
          {
            if ($condition->value_type == "Period")
            {
              ...
              switch ($app_list_strings['aor_sql_operator_list'][$condition->operator])
              {
                ...
                case "<=":
                  ...
                  $query['where'][] = $field . ' ' . $app_list_strings['aor_sql_operator_list'][$condition->operator]
                                      . ' "' . $sql_date2 . '"';
                  break;
              }
            }
            else
            {
              ...
              {
                $query['where'][] = ($tiltLogicOp ? '' : ($condition->logic_op ? $condition->logic_op . ' ' : 'AND '))
                                    . $field . ' ' . $app_list_strings['aor_sql_operator_list'][$condition->operator]
                                    . ' ' . $value;
              }
            }
          }
      ...
      $query['where'][] = $module->table_name . ".deleted = 0 "
                          . $this->build_report_access_query($module, $module->table_name);
