...
else
{
    ...
    if (!isset($_REQUEST['isTempFile']))
    {
        ...
        if (isset($focus->object_name) && $focus->object_name == 'Document')
        {
            ...
            $focusRevision->retrieve($image_id);
    ...
    {
        $local_location = sugar_cached("modules/Emails/{$_REQUEST['ieId']}/attachments/{$image_id}");
    }
    ...
    {
        $local_location = "upload://{$image_id}";
    }
    ...
    else
    {
        ...
        if ($file_type == 'documents')
        {
            ...
            $query .= "WHERE document_revisions.id = '" . $db->quote($image_id) . "' ";
        }
        ...
        elseif ($file_type == 'notes')
        {
            ...
            $query .= " WHERE notes.id = '" . $db->quote($image_id) . "'";
        }
        ...
        { //make sure not email temp file.
            ...
            $query = "SELECT " . $image_field . " name FROM " . $file_type . " ";
            if (substr($image_field, -2) == "_c")
            {
        ...
        if ($doQuery && isset($query))
        {
            ...
            //@todo: needs different error message string
            if (!isset($row['name']))
            {
                die($app_strings['ERROR_NO_RECORD']);
            }
            ...
            $download_location = "upload://{$image_id}"
                . (!empty($image_field) ? '_' . $image_field : '');
            /*
             * commented because $id_field is not defined anywhere
             *
